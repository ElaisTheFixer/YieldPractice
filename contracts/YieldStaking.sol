...
contract YieldStaking {
    ...
    function _createPlans() internal{
        ...
        _planOptions.push(stakePlans({apy: 2500, lockDuration: 7}));
        _planOptions.push(stakePlans({apy: 5000, lockDuration: 14}));
        _planOptions.push(stakePlans({apy: 7000, lockDuration: 30}));
        _planOptions.push(stakePlans({apy: 15000, lockDuration: 90}));
    }
    ...
    function upgradeStake(address _user) internal {
        uint256 currentApy = stakers[_user].apy;
        stakers[_user].apy = (currentApy * 2);
    }  
